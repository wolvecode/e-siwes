@props(['name' => $name, 'type' => $type, 'role'=> $role,
...
<div class="row mg-t-60">
    ...
    @if($na)
        <div class="card-header ml-3 pb-0">
            <p class="tx-14 tx-gray-500 mb-2">You can add multiple students by clicking <strong><a data-effect="effect-fall" data-target="#modalQuill" data-toggle="modal" href="{{route('application')}}">here</a></strong> <i>Note:</i> file upload must be excel or csv file</p>
        </div>
    @endif
...
{{--                            @if(!$na)--}}
                                <div class="form-group">
                                    <input type="email" name="email" class="form-control" id="inputEmail3" placeholder="Enter email" value="{{ old('email') }}">
                                </div>
                                @error('email')
                                <div class="alert alert-danger  mg-b-0" role="alert">
                                    <button aria-label="Close" class="close" data-dismiss="alert" type="button">
...
{{--                            @endif--}}
                            @if($na)
                                <x-matric :na="true" :type="'matric_no'" :action="'addStudent'"/>
